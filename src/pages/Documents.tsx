...
export const Documents: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const loadDocs = async () => {
      ...
      try {
        ...
        if (isMounted && data && data.success && Array.isArray(data.documents)) {
          const apiDocs: UserDocumentItem[] = data.documents.map((d: any) => ({
            id: d._id || d.id,
            fileName: d.originalFileName || d.cleanedFileName,
            fileType: (d.fileType || 'PDF').replace(/application\//i, '').toUpperCase(),
            fileSize: d.formattedSize || (d.fileSize ? `${Math.round(d.fileSize / 1024)} KB` : '1.2 MB'),
  ...
  const handleDownload = async (doc: UserDocumentItem, e: React.MouseEvent) => {
    e.stopPropagation();
    try {
    ...
    const file = new Blob([`=====================================================\n${doc.fileName} — CLEANED BY DOCUCLEAN AI\nStatus: Verified Pristine\n=====================================================\n\nRestored document content. All OCR noise and artifacts removed.`], {
      type: 'text/plain',
    });
